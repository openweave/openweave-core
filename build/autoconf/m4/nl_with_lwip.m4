...
[
    ...
    if test "$1" = 1; then
        ...
        AC_ARG_WITH(lwip,
            AS_HELP_STRING([--with-lwip=DIR],
                [Specify location of the LwIP headers and libraries, or "internal" to build using Weave's internal copy of LwIP @<:@default=internal@:>@.]),
            [
        ...
        AC_ARG_WITH(lwip-target,
            AS_HELP_STRING([--with-lwip-target=target],
                [Specify the target environment for which LwIP will be built.  Choose one of: standalone, none @<:@default=standalone@:>@.]),
            [
    ...
    # Define automake conditionals for building Weave with LwIP
    AM_CONDITIONAL([WEAVE_WITH_LWIP], [test "${1}" == 1])
