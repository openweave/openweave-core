...
__all__ = [ 'WeaveDeviceManager', 'NetworkInfo', 'DeviceDescriptor', 'DeviceManagerException' ]
...
class WeaveDeviceManager:
    ...
    def SetRendezvousAddress(self, addr):
        ...
        res = self._weaveStack.Call(
            lambda: self._dmLib.nl_Weave_DeviceManager_SetRendezvousAddress(self.devMgr, addr)
        )
        if (res != 0):
            raise self._weaveStack.ErrorToException(res)
    ...
    def Shutdown(self):
        self._weaveStack.Call(
            lambda: self._dmLib.nl_Weave_DeviceManager_Shutdown()
        )
    ...
    def _InitLib(self):
        if (self._dmLib == None):
            ...
            self._dmLib.nl_Weave_DeviceManager_Init.argtypes = [ ]
            self._dmLib.nl_Weave_DeviceManager_Init.restype = c_uint32

            self._dmLib.nl_Weave_DeviceManager_Shutdown.argtypes = [ ]
            self._dmLib.nl_Weave_DeviceManager_Shutdown.restype = c_uint32
        ...
        res = self._dmLib.nl_Weave_DeviceManager_Init()
        if (res != 0):
            raise self._weaveStack.ErrorToException(res)
