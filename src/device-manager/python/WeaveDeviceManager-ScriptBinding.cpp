...
extern "C" {
    ...
    NL_DLL_EXPORT WEAVE_ERROR nl_Weave_DataManagementClient_NewDataSink(WDMClient *wdmClient, uint16_t aResourceType,
            const uint8_t * aResourceId, size_t aResourceIdLen, uint64_t aInstanceId,
            const char * apPath, uint32_t aProfileId, PropertyInfo * apSchemaHandleTbl,
            ConstSchemaVersionRange * apVersionRange, uint32_t aNumSchemaHandleEntries,
            uint32_t aTreeDepth, GenericTraitUpdatableDataSink ** outGenericTraitUpdatableDataSink);
    ...
    NL_DLL_EXPORT WEAVE_ERROR nl_Weave_GenericTraitUpdatableDataSink_RefreshData(GenericTraitUpdatableDataSink * apGenericTraitUpdatableDataSink, WDMClient *wdmClient, WDMClientCompleteFunct onComplete, WDMClientErrorFunct onError);
    NL_DLL_EXPORT WEAVE_ERROR nl_Weave_GenericTraitUpdatableDataSink_SetBytes(GenericTraitUpdatableDataSink * apGenericTraitUpdatableDataSink, const char * apPath, const uint8_t * dataBuf, size_t dataLen, bool aIsConditional);
    NL_DLL_EXPORT WEAVE_ERROR nl_Weave_GenericTraitUpdatableDataSink_GetLeafBytes(GenericTraitUpdatableDataSink * apGenericTraitUpdatableDataSink, const char * apPath, ConstructBytesArrayFunct aCallback);
...
WEAVE_ERROR nl_Weave_DataManagementClient_NewWDMClient(WDMClient **outWDMClient, WeaveDeviceManager *devMgr)
{
    WEAVE_ERROR err = WEAVE_NO_ERROR;
...
WEAVE_ERROR nl_Weave_DataManagementClient_NewDataSink(WDMClient *wdmClient, uint16_t aResourceType,
        const uint8_t * aResourceId, size_t aResourceIdLen, uint64_t aInstanceId,
        const char * apPath, uint32_t aProfileId, PropertyInfo * apSchemaHandleTbl, uint32_t aNumSchemaHandleEntries,
        uint32_t aTreeDepth, ConstSchemaVersionRange * apVersionRange,
        GenericTraitUpdatableDataSink ** outGenericTraitUpdatableDataSink)
{
    ...
    err = wdmClient->NewDataSink(resourceId, aInstanceId, apPath, aProfileId, apSchemaHandleTbl,
            aNumSchemaHandleEntries, aTreeDepth, *outGenericTraitUpdatableDataSink);
    return err;
}
...
{
    WEAVE_ERROR err = WEAVE_NO_ERROR;
    err = wdmClient->FlushUpdate(NULL, NULL, onComplete, onError);
    return err;
}
...
{
    WEAVE_ERROR err = WEAVE_NO_ERROR;
    err = wdmClient->RefreshData(NULL, NULL, onComplete, onError, NULL);
    return err;
}

WEAVE_ERROR nl_Weave_GenericTraitUpdatableDataSink_Close(GenericTraitUpdatableDataSink * apGenericTraitUpdatableDataSink)
{
    if (apGenericTraitUpdatableDataSink != NULL)
    {
        apGenericTraitUpdatableDataSink->Close();
    }

    return WEAVE_NO_ERROR;
}

WEAVE_ERROR nl_Weave_GenericTraitUpdatableDataSink_RefreshData(GenericTraitUpdatableDataSink * apGenericTraitUpdatableDataSink, WDMClient * apWDMClient, WDMClientCompleteFunct onComplete, WDMClientErrorFunct onError)
{
    WEAVE_ERROR err = WEAVE_NO_ERROR;

    err = apGenericTraitUpdatableDataSink->RefreshData(NULL, apWDMClient, onComplete, onError);

    return err;
}

WEAVE_ERROR nl_Weave_GenericTraitUpdatableDataSink_SetBytes(GenericTraitUpdatableDataSink * apGenericTraitUpdatableDataSink, const char * apPath, const uint8_t * dataBuf, size_t dataLen, bool aIsConditional)
{
    WEAVE_ERROR err = WEAVE_NO_ERROR;
    err = apGenericTraitUpdatableDataSink->SetBytes(apPath, dataBuf, dataLen, aIsConditional);
    return err;
}

WEAVE_ERROR nl_Weave_GenericTraitUpdatableDataSink_GetLeafBytes(GenericTraitUpdatableDataSink * apGenericTraitUpdatableDataSink, const char * apPath, ConstructBytesArrayFunct aCallback)
{
    WEAVE_ERROR err = WEAVE_NO_ERROR;
    BytesData bytesData;
    err = apGenericTraitUpdatableDataSink->GetLeafBytes(apPath, &bytesData);
    SuccessOrExit(err);
