...
class NL_DLL_EXPORT GenericTraitDataSink : public nl::Weave::Profiles::DataManagement::TraitUpdatableDataSink
{
    friend class WDMClient;

private:
    GenericTraitDataSink(const nl::Weave::Profiles::DataManagement::TraitSchemaEngine * aEngine);
    ~GenericTraitDataSink(void);

public:
    void Close(void);

    WEAVE_ERROR RefreshData(WDMClient* apWDMClient, WDMClientCompleteFunct onComplete, WDMClientErrorFunct onError);

    WEAVE_ERROR SetData(const char * apPath, int8_t aValue, bool aIsConditional=false);
    WEAVE_ERROR SetData(const char * apPath, int16_t aValue, bool aIsConditional=false);
    WEAVE_ERROR SetData(const char * apPath, int32_t aValue, bool aIsConditional=false);
    WEAVE_ERROR SetData(const char * apPath, int64_t aValue, bool aIsConditional=false);
    WEAVE_ERROR SetData(const char * apPath, uint8_t aValue, bool aIsConditional=false);
    WEAVE_ERROR SetData(const char * apPath, uint16_t aValue, bool aIsConditional=false);
    WEAVE_ERROR SetData(const char * apPath, uint32_t aValue, bool aIsConditional=false);
    ...
    WEAVE_ERROR GetData(const char * apPath, int8_t& aValue);
    WEAVE_ERROR GetData(const char * apPath, int16_t& aValue);
    WEAVE_ERROR GetData(const char * apPath, int32_t& aValue);
    WEAVE_ERROR GetData(const char * apPath, int64_t& aValue);
    WEAVE_ERROR GetData(const char * apPath, uint8_t& aValue);
    WEAVE_ERROR GetData(const char * apPath, uint16_t& aValue);
    WEAVE_ERROR GetData(const char * apPath, uint32_t& aValue);
    ...
    WEAVE_ERROR GetLeafBytes(const char * apPath, ConstructBytesArrayFunct aCallback);
    WEAVE_ERROR GetBytes(const char * apPath, ConstructBytesArrayFunct aCallback);
...
public:
    ...
    WEAVE_ERROR NewDataSink(const ResourceIdentifier & aResourceId, uint32_t aProfileId, uint64_t aInstanceId, const char * apPath, GenericTraitDataSink *& apGenericTraitDataSink);

    WEAVE_ERROR FlushUpdate(void* appReqState, WDMClientCompleteFunct onComplete, WDMClientErrorFunct onError);

    WEAVE_ERROR RefreshData(void* appReqState, WDMClientCompleteFunct onComplete, WDMClientErrorFunct onError, GetDataHandleFunct getDataHandleCb);
...
private:
    ...
    WEAVE_ERROR GetDataSink(const ResourceIdentifier & aResourceId, uint32_t aProfileId, uint64_t aInstanceId, GenericTraitDataSink *& apGenericTraitDataSink);
