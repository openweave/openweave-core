    ...
    switch (aEvent)
    {
        ...
        case SubscriptionClient::kEvent_OnSubscribeRequestPrepareNeeded:
            ...
            VerifyOrExit(kOpState_RefreshData == savedOpState, err = WEAVE_ERROR_INCORRECT_STATE);
            {
                ...
                if (pWDMDMClient->mGetDataHandle != NULL && pWDMDMClient->mpContext != NULL)
                {
                    traitListLen = 1;
                    err = pWDMDMClient->mGetDataHandle(pWDMDMClient->mpContext, &pWDMDMClient->mSinkCatalog, handle);
                    SuccessOrExit(err);
...
WEAVE_ERROR WDMClient::FlushUpdate(void* apAppReqState, void* apContext, WDMClientCompleteFunct onComplete, WDMClientErrorFunct onError)
{
    WEAVE_ERROR err = WEAVE_NO_ERROR;
...
WEAVE_ERROR WDMClient::RefreshData(void* apAppReqState, void* apContext, WDMClientCompleteFunct onComplete, WDMClientErrorFunct onError, GetDataHandleFunct getDataHandleCb)
{
    WEAVE_ERROR err = WEAVE_NO_ERROR;
    VerifyOrExit(mOpState == kOpState_Idle, err = WEAVE_ERROR_INCORRECT_STATE);
