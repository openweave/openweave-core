...
GenericTraitUpdatableDataSink::GenericTraitUpdatableDataSink(const nl::Weave::Profiles::DataManagement::TraitSchemaEngine * aEngine)
...
WEAVE_ERROR GenericTraitUpdatableDataSink::RefreshData(void* appReqState, WDMClient* apWDMClient, WDMClientCompleteFunct onComplete, WDMClientErrorFunct onError)
{
    return apWDMClient->RefreshData(appReqState, this, onComplete, onError, LocateTraitHandle);
}
...
WEAVE_ERROR GenericTraitUpdatableDataSink::GetLeafBytes(const char * apPath, ConstructBytesArrayFunct aCallback)
{
    WEAVE_ERROR err = WEAVE_NO_ERROR;
    nl::Weave::TLV::TLVReader reader;
    PacketBuffer *pMsgBuf = NULL;
...
WEAVE_ERROR GenericTraitUpdatableDataSink::GetBytes(const char * apPath, ConstructBytesArrayFunct aCallback)
{
    WEAVE_ERROR err = WEAVE_NO_ERROR;
    ...
    switch (aEvent)
    {
        ...
        case SubscriptionClient::kEvent_OnSubscribeRequestPrepareNeeded:
            ...
            VerifyOrExit(kOpState_RefreshData == savedOpState, err = WEAVE_ERROR_INCORRECT_STATE);
            {
                ...
                if (pWDMDMClient->mGetDataHandle != NULL && pWDMDMClient->mpAppState != NULL)
                {
                    traitListLen = 1;
                    err = pWDMDMClient->mGetDataHandle(pWDMDMClient->mpAppState, &pWDMDMClient->mSinkCatalog, handle);
                    SuccessOrExit(err);
        ...
        case SubscriptionClient::kEvent_OnSubscriptionEstablished:
            ...
            pWDMDMClient->mOnComplete.General(pWDMDMClient, pWDMDMClient->mpAppReqState);
            break;
...
exit:
    if (WEAVE_NO_ERROR != err)
    {
        ...
        pWDMDMClient->mOnError(pWDMDMClient, pWDMDMClient->mpAppReqState, err, NULL);
    }
...
WEAVE_ERROR WDMClient::FlushUpdate(void* appReqState, void* aAppState, WDMClientCompleteFunct onComplete, WDMClientErrorFunct onError)
{
    WEAVE_ERROR err = WEAVE_NO_ERROR;

    VerifyOrExit(mOpState == kOpState_Idle, err = WEAVE_ERROR_INCORRECT_STATE);

    mpAppReqState = appReqState;
...
exit:
    ...
    {
        WeaveLogError(DataManagement, "FlushUpdate with: err = %d with OpState %d", err, mOpState);
        ClearOpState();
        mpAppReqState = NULL;
    }
...
WEAVE_ERROR WDMClient::RefreshData(void* appReqState, void* aAppState, WDMClientCompleteFunct onComplete, WDMClientErrorFunct onError, GetDataHandleFunct getDataHandleCb)
{
    WEAVE_ERROR err = WEAVE_NO_ERROR;
    VerifyOrExit(mOpState == kOpState_Idle, err = WEAVE_ERROR_INCORRECT_STATE);

    mpAppState = aAppState;
    mpAppReqState = appReqState;
    mOnComplete.General = onComplete;
    mOnError = onError;
...
exit:
    if (err != WEAVE_NO_ERROR)
    {
        WeaveLogError(DataManagement, "RefreshData with: err = %d with OpState %d", err, mOpState);
        ClearOpState();
        mpAppReqState = NULL;
    }
