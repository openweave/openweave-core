...
template <typename T>
GenericTraitCatalogImpl<T>::GenericTraitCatalogImpl()
        : mNodeId(0),
          mNextHandle(0)
{
    // Nothing to do.
...
template <typename T>
WEAVE_ERROR
GenericTraitCatalogImpl<T>::Clear ()
{
    WEAVE_ERROR err = WEAVE_NO_ERROR;
    typename Handle2ItemMap_t::iterator it;
    CatalogItem *item = NULL;
...
GenericTraitCatalogImpl<T>::AddressToHandle (TLV::TLVReader &aReader,
                                         ...
                                         SchemaVersionRange &aSchemaVersionRange) const
{
    nlPRECONDITION_VALUE(mNodeId != 0, WEAVE_ERROR_WRONG_NODE_ID);
...
template <typename T>
uint32_t GenericTraitCatalogImpl<T>::Count() const
{
    uint32_t count = 0;
    typename Handle2ItemMap_t::const_iterator it;
...
template <typename T>
WEAVE_ERROR GenericTraitCatalogImpl<T>::PrepareSubscriptionPathList(TraitPath * pathList, uint16_t pathListSize, uint16_t & pathListLen)
{
    WEAVE_ERROR err = WEAVE_NO_ERROR;
    typename Handle2ItemMap_t::const_iterator it;
    pathListLen = 0;
