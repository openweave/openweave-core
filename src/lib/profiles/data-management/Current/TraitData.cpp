...
WEAVE_ERROR TraitSchemaEngine::MapPathToHandle(const char * aPathString, PropertyPathHandle & aHandle) const
{
    ...
    char *token = NULL;
    char *pathStringCopy = (char *)malloc(strlen(aPathString) + 1);
    VerifyOrExit(pathStringCopy != NULL, err = WEAVE_ERROR_NO_MEMORY);

    strcpy(pathStringCopy, aPathString);
    token = strtok(pathStringCopy, "/");
    ...
    while (token!= NULL)
    {
        const uint64_t tag = atoi(token) | kSpecialTagMarker;
        WeaveLogDetail(DataManagement, "current tag is %d", tag);
        ...
        if (IsProfileTag(tag))
        {
            WeaveLogDetail(DataManagement, "it is profile tag ");
        ...
        {
            WeaveLogDetail(DataManagement, "looking for child tag");
            childProperty = GetChildHandle(curProperty, TagNumFromTag(tag));
        }

        WeaveLogDetail(DataManagement, "childProperty is %d", childProperty);
...
WEAVE_ERROR TraitUpdatableDataSource::StoreDataElement(PropertyPathHandle aHandle, TLVReader & aReader, uint8_t aFlags, void * aContext, TraitDataHandle aDatahandle, uint64_t aPreviousFirstSameTraitRequiredVersion)
{
    WEAVE_ERROR err = WEAVE_NO_ERROR;
    DataElement::Parser parser;
