 ...
 */
WEAVE_ERROR WeaveCertProvEngine::Init(Binding * binding, WeaveNodeOpAuthDelegate * opAuthDelegate, WeaveNodeManufAttestDelegate * manufAttestDelegate, EventCallback eventCallback, void * appState)
{
 ...
 *  @param[in]  doManufAttest        A boolean flag that indicates whether protocol should
 *                                   include manufacturer attestation data.
 *
 *  @retval #WEAVE_NO_ERROR          If StartCertificateProvisioning() was processed successfully.
 */
WEAVE_ERROR WeaveCertProvEngine::StartCertificateProvisioning(uint8_t reqType, bool doManufAttest)
{
    WEAVE_ERROR err;
 ...
 * via WeaveNodeOpAuthDelegate and WeaveNodeManufAttestDelegate functions and
 ...
 *  @param[in]  reqType              Get certificate request type.
 *  @param[in]  doManufAttest        A boolean flag that indicates whether request should
 *                                   include manufacturer attestation data.
 *
 *  @retval #WEAVE_NO_ERROR          If GetCertificateRequest was successfully generated.
 */
WEAVE_ERROR WeaveCertProvEngine::GenerateGetCertificateRequest(PacketBuffer * msgBuf, uint8_t reqType, bool doManufAttest)
{
    WEAVE_ERROR err = WEAVE_NO_ERROR;
    TLVWriter writer;
    ...
    if (doManufAttest)
    {
        VerifyOrExit(mManufAttestDelegate != NULL, err = WEAVE_ERROR_INVALID_ARGUMENT);
    }
    ...
    if (doManufAttest)
    {
        err = mManufAttestDelegate->EncodeMAInfo(writer);
        SuccessOrExit(err);
    }
    ...
    if (doManufAttest)
    {
        err = mManufAttestDelegate->GenerateAndEncodeMASig(tbsDataStart, tbsDataLen, writer);
        SuccessOrExit(err);
    }
...
WEAVE_ERROR WeaveCertProvEngine::SendGetCertificateRequest(void)
{
    ...
    if (mBinding->IsReady())
    {
        ...
        err = GenerateGetCertificateRequest(msgBuf, mReqType, mDoManufAttest);
        SuccessOrExit(err);
...
/**
 * @fn WeaveNodeManufAttestDelegate * WeaveCertProvEngine::GetManufAttestDelegate(void) const
 *
 * Returns a pointer to the manufacturer attestation delegate object currently configured on the WeaveCertProvEngine object.
 */

/**
 * @fn void WeaveCertProvEngine::SetManufAttestDelegate(WeaveNodeManufAttestDelegate * manufAttestDelegate)
 *
