 ...
 *                              be read.
 * @param[in] tag               TLV tag expected to be associated with the encoded signature structure.
 *
 * @retval #WEAVE_NO_ERROR      If the operation succeeded.
 * @retval other                Other Weave error codes related to signature reading.
 */
WEAVE_ERROR EncodedRSASignature::ReadSignature(TLVReader& reader, uint64_t tag)
{
    WEAVE_ERROR err;

    VerifyOrExit(reader.GetType() == kTLVType_ByteString, err = WEAVE_ERROR_WRONG_TLV_TYPE);
    VerifyOrExit(reader.GetTag() == tag, err = WEAVE_ERROR_INVALID_TLV_TAG);
                                               ...
                                               const uint8_t * keyDER, uint16_t keyDERLen)
{
    ...
    VerifyOrExit(sigAlgoOID == ASN1::kOID_SigAlgo_SHA256WithRSAEncryption, err = WEAVE_ERROR_UNSUPPORTED_SIGNATURE_TYPE);

    rsa = d2i_RSAPrivateKey(NULL, &keyDER, keyDERLen);
    VerifyOrExit(rsa != NULL, err = WEAVE_ERROR_NO_MEMORY);

    if (RSA_sign(NID_sha256, hash, hashLen, sigBuf, &sigLen, rsa) == 0)
                               ...
                               const uint8_t * certDER, uint16_t certDERLen)
{
    ...
    res = RSA_verify(NID_sha256, hash, hashLen, sig.Sig, sig.Len, EVP_PKEY_get1_RSA(pubKey));
