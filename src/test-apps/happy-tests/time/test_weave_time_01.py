...
#
#    @file
#       Calls Weave Time Sync test between nodes.
#
...
class test_weave_time_01(unittest.TestCase):
    ...
    def test_weave_time(self):
        ...
        value, data = self.__run_time_test_between("node01", "node02", "node03", "local")
        self.__process_result("node01", "node02", "node03", "local", value)

        value, data = self.__run_time_test_between("node01", "node02", "node03", "service")
        self.__process_result("node01", "node02", "node03", "service", value)

        value, data = self.__run_time_test_between("node01", "node02", "node03", "auto")
        self.__process_result("node01", "node02", "node03", "auto", value)

    def __process_result(self, nodeA, nodeB, nodeC, mode, value):
        print "time sync test among client:" + nodeA + ", coordinator:" + nodeB + ", server:" + nodeC + ", sync mode:" + mode

        if value:
            print hgreen("Passed")
        else:
            print hred("Failed")
            raise ValueError("Weave Time Sync Test Failed")
