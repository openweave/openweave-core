...
#
#    @file
#       Implements WeaveInet class that tests Inet Layer multicast
...
class WeaveInetMulticast(HappyNode, HappyNetwork, WeaveTest):
    """
    ...
    def __start_node(self, node, attributes, extra, tag):
        cmd = "sudo "
        cmd += self.getWeaveInetLayerMulticastPath()
        ...
        self.logger.debug("[localhost] WeaveInetMulticast: Will start process on node '%s' with tag '%s' and command '%s'" % (node, tag, cmd))
    ...
    def __start_receivers(self):
        ...
        for receiver in self.configuration['receivers']:
            self.__start_receiver(receiver, self.configuration['receivers'][receiver], identifier)
            identifier += 1            
    ...
    def run(self):
        ...
        self.logger.debug("[localhost] WeaveInetMulticast: Run.")

        self.__pre_check()

        self.__start_receivers()

        self.logger.debug("[%s] WeaveInetMulticast: %u receivers should be running." % ("localhost", len(self.receivers)))

        for receiver in self.receivers:
            emsg = "receiver %s should be running." % (receiver['tag'])
            self.logger.debug("[%s] WeaveInetMulticast: %s" % (receiver['node'], emsg))
