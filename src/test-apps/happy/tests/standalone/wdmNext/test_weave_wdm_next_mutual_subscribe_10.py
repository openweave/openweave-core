...
#       F06: Mutual Subscribe: Root path, Null Version. Mutate data in initiator. Publisher in initiator cancels
...
class test_weave_wdm_next_mutual_subscribe_10(weave_wdm_next_test_base):

    def test_weave_wdm_next_mutual_subscribe_10(self):
        wdm_next_args = {}
        ...
        wdm_next_args['client_log_check'] = [('Client\[0\] \[(ALIVE|CONFM)\] bound mutual subscription is going away', wdm_next_args['test_client_iterations']),
                                             ('Handler\[0\] \[ALIVE\] EndSubscription Ref\(\d+\)', wdm_next_args['test_client_iterations']),
