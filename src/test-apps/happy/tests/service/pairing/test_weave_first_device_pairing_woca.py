...
TESTRAIL_SECTION_NAME = "Weave first device pairing between Mock-Client and Real-Service"
TESTRAIL_SUFFIX = "_TESTRAIL.json"
DEFAULT_FABRIC_SEED = "00001"


class test_weave_first_device_pairing_woca(unittest.TestCase):
    def setUp(self):
        self.tap = None
        self.tap_id = None
        ...
        self.eui64_prefix = os.environ.get("EUI64_PREFIX", '1A:B4:30:AA:00:')
...
#        value, data = self.__stop_tunnel_from("BorderRouter")
    ...
    def test_weave_pairing(self):
        ...
        value, data = self.__run_pairing_test_between("BorderRouter", "mobile")
...
#        options["devices"] = [nodeA + str(index + self.initial_device_index).zfill(2) for index in range(self.device_numbers)]
        options["devices"] = nodeA
        options["server"] = "service"
        options["service_dir"] = self.use_service_dir
        options["woca_server"] = "service"
        options["tun_server"] = "service"
        options["case"] = True
        options["tap"] = self.tap_id
