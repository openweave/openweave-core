...
{
  return tcp_create_segment(&pcb->remote_ip.ip4, &pcb->local_ip.ip4, pcb->remote_port, pcb->local_port,
...
{
  return tcp_create_segment_wnd(&pcb->remote_ip.ip4, &pcb->local_ip.ip4, pcb->remote_port, pcb->local_port,
                   ...
                   ip_addr_t* remote_ip, u16_t local_port, u16_t remote_port)
{
  ...
  if (state == ESTABLISHED) {
    ...
    pcb->local_ip.ip4.addr = local_ip->addr;
    pcb->local_port = local_port;
    pcb->remote_ip.ip4.addr = remote_ip->addr;
...
void test_tcp_input(struct pbuf *p, struct netif *inp)
{
  ...
  ip_addr_copy(ipX_current_dest_addr()->ip4, iphdr->dest);
  ip_addr_copy(ipX_current_src_addr()->ip4, iphdr->src);
  ...
  ipX_current_dest_addr()->ip4.addr = 0;
  ipX_current_src_addr()->ip4.addr = 0;
